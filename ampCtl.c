...
int main(int argc, char **argv, char **envp)
{
	...
	int nfds = 1;
	int gpio_fd, gpio_fdMute, gpio_fdAmp; 
	int timeout, rc;
	char buf = 0;
	unsigned int gpio, gpioMute, gpioAmp;
	int len;
	int value;
	int init = 1;
	...
	gpioAmp = atoi(argv[2]);
	gpio_export(gpioAmp);
	gpio_set_dir(gpioAmp, 1);
	gpio_fdAmp = gpio_fd_open(gpioAmp);


	gpioMute = atoi(argv[3]);
	gpio_export(gpioMute);
	gpio_set_dir(gpioMute, 1);
	gpio_fdMute = gpio_fd_open(gpioMute);
		...
		if (delay(&prev, &cur) < AMP_DEBOUNCE) continue;
                ...
                if (buf == '1') {  /* The button has been pressed */
                        if (stateAmp == 0) { /* Amp is currently off -> switch on */
                                stateAmp = 1;
                                stateMute = 0;
				ampState(stateAmp, gpio_fdAmp);
				ampMute(stateMute, gpio_fdMute);
                        }
